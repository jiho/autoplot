...
# # Extraction of supplementary data in FactoMineR
# col_scores.PCA <- function(x, eig, ...) {
#   res <- data.frame()
#   kind <- c()
...
# (prop_eig_2 * const)
#
# rs0 <- vegan:::scores.rda(xV, 1:4, display = c("sites"), scaling=0)
# rs1 <- vegan:::scores.rda(xV, 1:4, display = c("sites"), scaling=1)
# rs2 <- vegan:::scores.rda(xV, 1:4, display = c("sites"), scaling=2)
# rs3 <- vegan:::scores.rda(xV, 1:4, display = c("sites"), scaling=3)
#
# all.equal((rs1 / rs0)[1,], prop_eig_2 * const, check.attributes=F)
# all.equal((rs2 / rs0)[1,], rep(const, 4), check.attributes=F)
# all.equal((rs3 / rs0)[1,], prop_eig_4 * const, check.attributes=F)
#
# cs0 <- vegan:::scores.rda(xV, 1:4, display = c("species"), scaling=0)
# cs1 <- vegan:::scores.rda(xV, 1:4, display = c("species"), scaling=1)
# cs2 <- vegan:::scores.rda(xV, 1:4, display = c("species"), scaling=2)
# cs3 <- vegan:::scores.rda(xV, 1:4, display = c("species"), scaling=3)
#
# all.equal((cs1 / cs0)[1,], rep(const, 4), check.attributes=F)
# all.equal((cs2 / cs0)[1,], prop_eig_2 * const, check.attributes=F)
# all.equal((cs3 / cs0)[1,], prop_eig_4 * const, check.attributes=F)
#
# # FACTOMINER
# rs <- xF$ind$coord
# cs <- xF$var$coord
...
# n <- nrow(rs)
#
# all.equal((rs / rs0)[1,], sqrt(eig * n), check.attributes=F)
# all.equal((cs / cs0)[1,], sqrt(eig), check.attributes=F)
#
#
# # ADE4
